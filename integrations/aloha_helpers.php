...
function schedule_aloha_process_batch_job(PDO $pdo, int $restaurantId, int $batchId): array
{
    $batchStmt = $pdo->prepare('SELECT * FROM aloha_import_batches WHERE restaurant_id=:restaurant_id AND id=:id');
    $batchStmt->execute([':restaurant_id' => $restaurantId, ':id' => $batchId]);
    $batch = $batchStmt->fetch(PDO::FETCH_ASSOC);
    if (!is_array($batch)) {
        throw new RuntimeException('Batch not found.');
    }
    if ((string)$batch['status'] === 'processed') {
        return ['rows_total' => 0, 'rows_imported' => 0, 'rows_skipped' => 0, 'errors_count' => 0, 'top_errors' => []];
    }

    $meta = json_decode((string)($batch['mapping_json'] ?? '{}'), true);
    if (!is_array($meta) || !isset($meta['mapping']) || !is_array($meta['mapping'])) {
        throw new RuntimeException('Save a field mapping before processing.');
    }
    $mapping = $meta['mapping'];

    $path = schedule_aloha_batch_file_path($batchId);
    if (!is_file($path)) {
        throw new RuntimeException('CSV file is missing from storage.');
    }

    $importType = (string)$batch['import_type'];
    if ($importType === 'employees') {
        $clear = $pdo->prepare('DELETE FROM aloha_employees_stage WHERE restaurant_id=:restaurant_id AND batch_id=:batch_id');
    } elseif ($importType === 'labor') {
        $clear = $pdo->prepare('DELETE FROM aloha_labor_punches_stage WHERE restaurant_id=:restaurant_id AND batch_id=:batch_id');
    } else {
        $clear = $pdo->prepare('DELETE FROM aloha_sales_daily_stage WHERE restaurant_id=:restaurant_id AND batch_id=:batch_id');
    }
    $clear->execute([':restaurant_id' => $restaurantId, ':batch_id' => $batchId]);
    ...
    $status = $summary['errors_count'] > 0 && $summary['rows_imported'] === 0 ? 'failed' : 'processed';
    $stmt = $pdo->prepare('UPDATE aloha_import_batches SET status=:status, processed_at=NOW(), error_text=:error_text WHERE restaurant_id=:restaurant_id AND id=:id');
    $stmt->execute([
        ':status' => $status,
...
@@ -223,204 +393,81 @@ function schedule_handle_aloha_api(string $action, int $resId, int $userId): voi
    ...
    if ($action === 'aloha_process_batch' || $action === 'aloha_queue_process_batch') {
        ...
        $jobId = jq_enqueue(
            $pdo,
            'aloha_process_batch',
            ['batch_id' => $batchId, 'restaurant_id' => $resId, 'import_type' => (string)$batch['import_type']],
            50,
            $resId,
            $userId,
            0
        );
        schedule_execute('UPDATE aloha_import_batches SET status="queued" WHERE restaurant_id=:restaurant_id AND id=:id', [
            ':restaurant_id' => $resId,
            ':id' => $batchId,
        ]);
